...
func _verify_role(role: String, identity: String, minimum_height: float, maximum_height: float) -> void:
	...
	_check(_has_named_layer(visual, "nativehead"), "%s lacks its native modeled head" % role)
	_check(_has_named_layer(visual, "peasantoutfit"), "%s lacks its complete outfit body" % role)
	_check(_has_named_layer(visual, "nativehair"), "%s lacks bone-driven native hair" % role)
	_check(_count_type(visual, "Skeleton3D") >= 3, "%s composition layers are not rigged" % role)
	_check(_count_type(visual, "AnimationPlayer") >= 3, "%s composition layers do not share animation" % role)
