...
func _initialize() -> void:
	if DisplayServer.get_name().to_lower() == "headless":
		push_error("CHAR-008 capture requires a graphical renderer")
		quit(1)
		return
	...
	var specs := [["mira_human", "mira_herbalist", -2.4], ["rook_human", "rook_smuggler", -0.8], ["castle_guard_human", "castle_guard", 0.8], ["villager_female_human", "widow_elna", 2.4]]
	for item in specs:
	...
	stage.queue_free()
	await _frames(3)
	print("CHAR-008 CAPTURE: %s" % ("PASS" if failures == 0 else "FAIL (%d)" % failures))
