...
func _initialize() -> void:
	if DisplayServer.get_name().to_lower() == "headless":
		push_error("WORLD-003 capture requires a graphical renderer")
		quit(1)
		return
	...
	game.call("_new_game")
	await _frames(60)
	await _capture(game, "WORLD_003_01_CemeteryApproach", Vector3(10.2, 1, 8.0), -1.35)
	await _capture(game, "WORLD_003_02_GraveCourt", Vector3(12.4, 1, 9.4), -1.15)
	await _capture(game, "WORLD_003_03_RuinedCrowChapel", Vector3(14.3, 1, 8.1), -1.45)
	await _capture(game, "WORLD_003_04_CastleGateway", Vector3(15.8, 1, 1.2), -1.30)
	print("WORLD-003 SCREENSHOTS: %s" % ("PASS" if failures == 0 else "FAIL (%d)" % failures))
...
func _capture(game, stem: String, position: Vector3, yaw: float) -> void:
	...
	if image == null or image.get_width() != 1280 or image.get_height() != 720:
		failures += 1
		push_error("Invalid WORLD-003 frame: %s" % stem)
		return
	...
	if luminance_max - luminance_min < 0.08:
		failures += 1
		push_error("Blank WORLD-003 frame: %s" % stem)
		return
...
func _read_stable_frame() -> Image:
	...
	for _attempt in range(4):
		...
		if image != null and image.get_width() == 1280 and image.get_height() == 720:
			return image
		await process_frame
	return image
