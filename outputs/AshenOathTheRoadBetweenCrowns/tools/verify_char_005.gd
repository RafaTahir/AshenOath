...
func _initialize() -> void:
	...
	for role_id in ROLE_PATHS:
		...
		if skeletons.size() == 1:
			...
			_check(_has_any_bone(skeleton, ["head", "Head_2"]), "%s lacks a head bone" % role_id)
			_check(_has_any_bone(skeleton, ["hand_r", "Hand.R", "Weapon.R", "Fist.R"]), "%s lacks a right-hand equipment bone" % role_id)
			_check(_has_any_bone(skeleton, ["hand_l", "Hand.L", "Fist.L"]), "%s lacks a left-hand equipment bone" % role_id)
			var skinned := 0
			for mesh in meshes:
				if (mesh as MeshInstance3D).skin != null:
					skinned += 1
			_check(skinned > 0, "%s has no skinned body mesh" % role_id)
...
func _has_any_bone(skeleton: Skeleton3D, aliases: Array[String]) -> bool:
	for index in range(skeleton.get_bone_count()):
		var name := str(skeleton.get_bone_name(index)).to_lower().replace("_", "").replace(".", "")
		for alias in aliases:
			var wanted := alias.to_lower().replace("_", "").replace(".", "")
